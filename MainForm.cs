...
namespace SampleApplication
{
  public partial class MainForm : Form
  {
    ...
    // TCP Client for AlignSight Command Communication 
    ...
    private int RsltChangedCount = 0;
    ...
    private void btn_Connect_Click(object sender, EventArgs e)
    {
      ...
      try
      {
        mCurrentSystem = AlignSystemFactory.Create(IPAddress.Parse(txtBox_IPAddress.Text));
      }
      catch (ArgumentException ex)           //"The provided IP Address could not be resolved."
      {
        toolStripStatusLabelStatus.Text = "Error : " + ex.Message;
        return;
      }
    ...
    //  show_CurrentSystem_onHMI();
      ...
      if(chkAttachEvent.Checked == true )
      {
        mCurrentSystem.JobLoaded += OnJobLoaded;
        mCurrentSystem.ResultsChanged += OnResultsChanged;
        RsltChangedCount = 0;
    ...
    /// <summary>
    /// ResultsChanged event handler. 
    /// It also occurs when switching recipes with the Native command.
    /// </summary>
    ...
    {
      RsltChangedCount++;
      lblRsltChangedCount.Text = RsltChangedCount.ToString();
      //MessageBox.Show("ResultsChanged" + e.ToString());

      // This method resets the recipe switched by the Native command as Current Recipe.
      mCurrentSystem?.UpdateSelectedRecipe();
      updateCurretRecipeInfo();
    }

    /// <summary>
    /// JobLoaded event handler. 
    /// This event does not occur when the recipe is loaded with the Native command.
    /// </summary>
    /// <param name="sender"></param>
    /// <param name="e"></param>
    private void OnJobLoaded(object sender, EventArgs e)
    {
      LoadCompletedCount++;
      lblLoadCompletedCount.Text = LoadCompletedCount.ToString();
      //MessageBox.Show("JobLoaded" + e.ToString());
    }
    ...
    private void btn_Disconnect_Click(object sender, EventArgs e)
    {
      ...
      mCurrentSystem = null;
      show_CurrentSystem_onHMI();        //clear fields in Current System on HMI
    }
    #endregion   // Manual Connection
    ...
    private void btn_ChangeCamNetInfo_Click(object sender, EventArgs e)
    {
      ...
      if (manualPowerCycle)
      {
        ...
        FormPowerCycleDialog form2 = new FormPowerCycleDialog(cancelToken);
        form2.TopMost = true;
        form2.ShowDialog();        //wait untl windows is closed.
    ...
    private void btn_CreateSys_Click(object sender, EventArgs e)
    {
      ((Button)sender).BackColor = Color.Red;
      if (string.IsNullOrEmpty(txtBox_NewSysName.Text))
      {
        toolStripStatusLabelStatus.Text = "Error : New System Name is required";
        return;
      }

      ICameraNetworkInfo selectedCameraInfo = null;
      List<ICameraNetworkInfo> camerasToBeUsed = new List<ICameraNetworkInfo>();
      foreach (ListViewItem item in list_Cameras.SelectedItems)
      {
        if (item.Group.Name == list_Cameras.Groups[1].Name)
        {
          selectedCameraInfo = mAlignSystemManager.GetUnusedCameraInfos().Where(camInfo => camInfo.Name == item.Text).FirstOrDefault();
          camerasToBeUsed.Add(selectedCameraInfo);
        }
      }

      if (checkBox_LowerIsMaster.Checked)
        camerasToBeUsed.Reverse();

      if (camerasToBeUsed.Count < 1 || camerasToBeUsed.Count > 2)
      {
        toolStripStatusLabelStatus.Text = "Error : Number of unconfigured cameras needs to be 1 or 2";
        //                return;
      }

      mAlignSystemManager.NewAlignSystem(txtBox_NewSysName.Text, txtBox_NewSysDescription.Text, camerasToBeUsed);

      //Clear fields on HMI
      txtBox_NewSysName.Text = "";
      txtBox_NewSysDescription.Text = "";
      checkBox_LowerIsMaster.Checked = false;
      btn_Update_CamList_CurrSysInfo.PerformClick();
      ((Button)sender).BackColor = Color.Lime;
    }
    ...
    private void updateSystemSettingsTab()       //called by initForChangedSystem()
    {
      ...
      // General Settings
      checkBox_ShuttledCamera.Checked = currSysSettings?.UseShuttledCameras ?? false;
      checkBox_MovableOrigin.Checked = currSysSettings?.UseMoveableOrigin ?? false;
      checkBox_RunUncalibrated.Checked = currSysSettings?.RunUncalibrated ?? false;
    ...
    private void btn_ChangeSysSettings_Click(object sender, EventArgs e)
    {
      ...
      tmpSysSettings.UseShuttledCameras = checkBox_ShuttledCamera.Checked;
      tmpSysSettings.UseMoveableOrigin = checkBox_MovableOrigin.Checked;
      tmpSysSettings.RunUncalibrated = checkBox_RunUncalibrated.Checked;
    ...
    private void btn_LoadRecipe_Click(object sender, EventArgs e)
    {
      ...
      {
        mCurrentSystem.LoadRecipe((string)list_Recipes.SelectedItem);
        updateCurretRecipeInfo();
        updateExposureTab();
        updateCheckerTab(1);
        updateFeatureTab(1,1);
      }
      btn_DisconnectCommand.PerformClick();          //Disonnect TCPIP connection for command
      ((Button)sender).BackColor = Color.Lime;
    }
    ...
    #region SNTP Settings ***************************************************************************
    private void updateSNTPSettingTab()
    {
      var SNTPSetting = mCurrentSystem.GetAlignSystemSettings().SNTP;
      chkSNTP.Checked = SNTPSetting.Enabled;
      chkSummerTime.Checked = SNTPSetting.AdjustForDayTimeSavings;
      txtSNTPServerIP.Text = SNTPSetting.Hostname;
      numSNTPInterval.Value = SNTPSetting.UpdateFrequency;
      txtUsedSNTPServerIP.Text = SNTPSettings.GetLocatedSNTPServer(mCurrentSystem);
      lblSNTPVersion.Text = SNTPSetting.Version.ToString();

      cbTimeZone.Items.Clear();
      var timeZones = SNTPSettings.GetAllTimeZones(mCurrentSystem);
      foreach (var timeZone in timeZones)
      {
        cbTimeZone.Items.Add(timeZone);
      }
      cbTimeZone.SelectedIndex = SNTPSetting.TimeZoneID - 1;
    }

    private void btnSNTPSetting_Click(object sender, EventArgs e)
    {
      ((Button)sender).BackColor = Color.Red;
      var settings = mCurrentSystem.GetAlignSystemSettings();

      settings.SNTP.Enabled = chkSNTP.Checked;
      settings.SNTP.TimeZoneID = SNTPSettings.GetTimeZoneID(mCurrentSystem, cbTimeZone.SelectedIndex);
      settings.SNTP.AdjustForDayTimeSavings = chkSummerTime.Checked;
      settings.SNTP.Hostname = txtSNTPServerIP.Text;
      settings.SNTP.UpdateFrequency = Decimal.ToInt32(numSNTPInterval.Value);
      if (mCurrentSystem.IsConnected)
      {
        mCurrentSystem.SetAlignSystemSettings(settings);
      }
    ...
    #endregion    // SNTP Settings

    #region FTP Settings ***************************************************************************
    private void updateFTPSettingTab()
    {
      var DataSetting = mCurrentSystem.GetAlignSystemSettings().DataLogging;
      var ImageSetting = mCurrentSystem.GetAlignSystemSettings().ImageLogging;
      var RecipeSetting = mCurrentSystem.GetAlignSystemSettings().RecipeServer;
      var FTPSetting = mCurrentSystem.GetAlignSystemSettings().FTP;
      var NetworkSetting = mCurrentSystem.GetAlignSystemSettings().Network;

      txtFTPServer.Text = DataSetting.Hostname;
      txtFTPUser.Text = DataSetting.UserName;
      txtFTPPassword.Text = DataSetting.Password;
      numFTPPort.Value = DataSetting.Port;
      txtFTPDefaultGateway.Text = NetworkSetting.Gateway;
      chkFTPPassive.Checked = FTPSetting.PassiveTransfer;

      chkFTPRecipe.Checked = RecipeSetting.Enabled;
      chkFTPLog.Checked = DataSetting.Enabled;
      chkFTPImage.Checked = ImageSetting.Enabled;
      chkFTPImgNameDate.Checked = ImageSetting.UseDateInFileName;
      chkFTPImgNameTime.Checked = ImageSetting.UseTimeInFileName;
      chkFTPImgAlign.Checked = ImageSetting.LogProductionImages;
      chkFTPImgFail.Checked = ImageSetting.LogFailImages;
      chkFTPImgTrain.Checked = ImageSetting.LogTrainImages;
      chkFTPImgGraphics.Checked = ImageSetting.Graphics;
      cbFTPImgSize.SelectedItem = Enum.GetName(typeof(LogImageFTPSettingsImageResolutions), ImageSetting.ImageResolution);
    }
    ...
    private void btnFTP_Click(object sender, EventArgs e)
    {
      ((Button)sender).BackColor = Color.Red;
      var DataSetting = mCurrentSystem.GetAlignSystemSettings().DataLogging;
      var logImageSettings = mCurrentSystem.GetAlignSystemSettings().ImageLogging;
      var RecipeSetting = mCurrentSystem.GetAlignSystemSettings().RecipeServer;
      var FTPSetting = mCurrentSystem.GetAlignSystemSettings().FTP;
      var NetworkSetting = mCurrentSystem.GetAlignSystemSettings().Network;

      DataSetting.Hostname = txtFTPServer.Text;
      logImageSettings.Hostname = txtFTPServer.Text;
      RecipeSetting.Hostname = txtFTPServer.Text;
      DataSetting.UserName = txtFTPUser.Text;
      logImageSettings.UserName = txtFTPUser.Text; ;
      RecipeSetting.UserName = txtFTPUser.Text;
      DataSetting.Password = txtFTPPassword.Text;
      logImageSettings.Password = txtFTPPassword.Text;
      RecipeSetting.Password = txtFTPPassword.Text;
      DataSetting.Port = Decimal.ToInt32(numFTPPort.Value);
      logImageSettings.Port = Decimal.ToInt32(numFTPPort.Value);
      RecipeSetting.Port = Decimal.ToInt32(numFTPPort.Value);
      NetworkSetting.Gateway = txtFTPDefaultGateway.Text;
      FTPSetting.PassiveTransfer = chkFTPPassive.Checked;

      RecipeSetting.Enabled = chkFTPRecipe.Checked;
      DataSetting.Enabled = chkFTPLog.Checked;
      logImageSettings.Enabled = chkFTPImage.Checked;
      logImageSettings.UseDateInFileName = chkFTPImgNameDate.Checked;
      logImageSettings.UseTimeInFileName = chkFTPImgNameTime.Checked;
      logImageSettings.LogProductionImages = chkFTPImgAlign.Checked;
      logImageSettings.LogFailImages = chkFTPImgFail.Checked;
      logImageSettings.LogTrainImages = chkFTPImgTrain.Checked;
      logImageSettings.Graphics = chkFTPImgGraphics.Checked;
      logImageSettings.ImageResolution = (LogImageFTPSettingsImageResolutions)Enum.Parse(typeof(LogImageFTPSettingsImageResolutions), cbFTPImgSize.SelectedItem.ToString(), true);

      var system = mCurrentSystem.GetAlignSystemSettings();
      system.DataLogging = DataSetting;
      system.ImageLogging = logImageSettings;
      system.RecipeServer = RecipeSetting;
      system.FTP = FTPSetting;
      system.Network = NetworkSetting;
      if (mCurrentSystem.IsConnected)
      {
        mCurrentSystem.SetAlignSystemSettings(system);
      }
      else
      {
        MessageBox.Show("Not Connected");
    ...
    #region TCP Client for AlignSight Command Communication ******************************************************
    private void btn_Connect_Command_Click(object sender, EventArgs e)
    {
      try
      {
        tcpClient = new System.Net.Sockets.TcpClient(txtBox_IPAddress_Command.Text, Convert.ToInt16(txtBox_PortNumber_Command.Text));
      }
      catch (Exception ex)
      {
        toolStripStatusLabelStatus.Text = "Error : " + ex.Message.Substring(0, 80);
        return;
      }
      toolStripStatusLabelStatus.Text = "Connected " + txtBox_IPAddress_Command.Text + " : " + txtBox_PortNumber_Command.Text;

      netStream = tcpClient.GetStream();
      netStream.ReadTimeout = 10000;   //10sec"
      netStream.WriteTimeout = 10000;   //10sec"

      ((Button)sender).BackColor = Color.WhiteSmoke;
      btn_DisconnectCommand.BackColor = Color.Lime;
      btn_SendCommand.BackColor = Color.Lime;
    }

    private void btn_DisconnectCommand_Click(object sender, EventArgs e)
    {
      netStream?.Dispose();
      tcpClient?.Close();
      toolStripStatusLabelStatus.Text = "Disconnected " + txtBox_IPAddress_Command.Text + " : " + txtBox_PortNumber_Command.Text;

      btn_ConnectCommand.BackColor = Color.Lime;
      ((Button)sender).BackColor = Color.WhiteSmoke;
      btn_SendCommand.BackColor = Color.WhiteSmoke;
    }
    ...
    private void btn_SendCommand_Click(object sender, EventArgs e)
    {
      ...
      {
        toolStripStatusLabelStatus.Text = "Error : Not Connected.";
        return;
      }

      // Encode the data string into a byte array.",
      string msgString = txtBox_SentCommand.Text;
      byte[] msg = Encoding.ASCII.GetBytes(msgString + "\r\n");
      // Send the data through the socket.",
      netStream.Write(msg, 0, msg.Length);
      toolStripStatusLabelStatus.Text = "Sent : " + msgString;
      string msgString_noSpace = msgString.Replace(" ", "");
      txtBox_SentCommand.Text = msgString_noSpace.Replace(",", ", ");

      string data = null;
      byte[] bytes = new Byte[1024];
      while (true)
      {
        int bytesRec = netStream.Read(bytes, 0, bytes.Length);
        data += Encoding.ASCII.GetString(bytes, 0, bytesRec);  //adding each Byte to string"
        if (data.IndexOf("\r") > -1)      //if CR is found
          break;
      }
      txtBox_ReceivedResponse.Text = data.Replace(",", ", ");
    }
    ...
    {
      if (mCurrentSystem == null || mCamera1 == null)
        return;

      var dlg = new FormManualAlign(mCurrentSystem, mCamera1);
      dlg.ShowDialog(this);
    }
